...
contract MainAggregator is IHumanityOracle, Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 private constant SUSPICIOUS_THRESHOLD = 3;
    uint256 private constant WINDOW_DURATION = 1 hours;
    ...
    event VerificationRegistered(address indexed user, uint8 indexed source, bytes32 uniqueId, uint256 timestamp, uint256 qualityScore);
    ...
    function registerVerification(
        ...
        CompromiseWindow memory cw = compromiseWindows[source];
        if (cw.isCompromised && block.timestamp >= cw.startTime && block.timestamp <= cw.endTime) {
        ...
        if (window.startTime == 0 || block.timestamp > window.startTime + WINDOW_DURATION) {
            sourceWindows[source] = SourceWindow({startTime: block.timestamp, verificationCount: 1, suspiciousCount: 0});
        ...
        require(verificationToken.transfer(user, TOKEN_REWARD), "Token transfer failed");

        emit VerificationRegistered(user, source, uniqueId, block.timestamp, qualityScore);
    }
    ...
    function isVerifiedHuman(address _address) external view returns (bool) {
        ...
        for (uint256 i = 0; i < verifications.length; i++) {
            if (isVerificationValid(_address, i)) return true;
        }
        return false;
    }
    ...
    function isVerificationValid(address user, uint256 index) public view returns (bool) {
        ...
        VerificationData memory v = userVerifications[user][index];
        if (invalidatedVerifications[v.uniqueId]) return false;
        CompromiseWindow memory cw = compromiseWindows[v.source];
        if (cw.isCompromised && v.timestamp >= cw.startTime && v.timestamp <= cw.endTime) return false;
        return true;
    }
    ...
    function getTrustScore(address _address) external view returns (uint256) {
        return getHumanProbability(_address) / 1e16;
    }
    ...
    function updateSourceConfidence(uint8 sourceId) public {
        ...
        uint256 newDenominator = conf.numerator + newFPR;
    ...
    function markSourceCompromised(uint8 sourceId, uint256 startTime, uint256 endTime) external onlyOwner {
        if (endTime <= startTime) revert InvalidTimeWindow();
        if (endTime > block.timestamp + 365 days) revert InvalidTimeWindow();
        compromiseWindows[sourceId] = CompromiseWindow({startTime: startTime, endTime: endTime, isCompromised: true});
        emit SourceCompromised(sourceId, startTime, endTime);
    }

    function resetSourceWindow(uint8 sourceId) external onlyOwner {
        sourceWindows[sourceId] = SourceWindow({startTime: block.timestamp, verificationCount: 0, suspiciousCount: 0});
        emit SourceWindowReset(sourceId);
    }

    function withdrawTokens(uint256 amount) external onlyOwner nonReentrant {
        require(verificationToken.transfer(owner(), amount), "Token transfer failed");
    }
