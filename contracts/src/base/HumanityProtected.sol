...
abstract contract HumanityProtected {
    ...
    modifier onlyHuman() {
        if (!humanityOracle.isVerifiedHuman(msg.sender)) revert NotVerifiedHuman(msg.sender);
        _;
    }

    modifier minTrustScore(uint256 required) {
        uint256 actual = humanityOracle.getTrustScore(msg.sender);
        if (actual < required) revert InsufficientTrustScore(msg.sender, required, actual);
        _;
    }
